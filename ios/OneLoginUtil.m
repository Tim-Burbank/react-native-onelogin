...
@property (nonatomic, copy) NSDictionary *privaryItems;

@end

@implementation OneLoginUtil

RCT_EXPORT_MODULE(OneLogin);


RCT_EXPORT_METHOD(dismissAuth) {
    dispatch_async(dispatch_get_main_queue(), ^{
        self.resolve = nil;
        self.reject = nil;
        [OneLogin dismissAuthViewController:YES completion:nil];
    });
}

RCT_EXPORT_METHOD(setAdditionalPrivacyTerms:(NSDictionary *)items) {
    self.privaryItems = items;
}
...
RCT_EXPORT_METHOD(ongLoginInit:(NSString *)appid) {
    [OneLogin registerWithAppID:appid];
}

RCT_EXPORT_METHOD(preRequest:(NSString *)appkey degbug:(BOOL)isDebug resolve:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject) {
    self.resolve = resolve;
    self.reject = reject;
    if (![OneLogin hasRegistered]) {
        [OneLogin registerWithAppID:appkey];
    }
    if ([self needPreGetToken]) {
        [OneLogin preGetTokenWithCompletion:^(NSDictionary * _Nonnull preResult) {
            if (preResult.count > 0 && preResult[@"status"] && 200 == [preResult[@"status"] integerValue]) {
                self.resolve(@{@"status": @200});
            } else {
                self.reject(preResult[@"status"] ? @"-2222" : preResult[@"status"], @"prerquest fail", nil);
            }
        }];
    } else {
        self.resolve(@{@"status": @200});
...
RCT_EXPORT_METHOD(requestToken:(NSString *)appkey debug:(BOOL)isDebug resolve:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject) {
    self.resolve = resolve;
    self.reject = reject;
    if ([OneLogin hasRegistered]) {
        [self floatWindowLogin];
    } else {
        [OneLogin registerWithAppID:appkey];
        [self floatWindowLogin];
    }
...
- (void)normalLogin:(UIViewController *)controller {
    ...
    viewModel.sloganTextFont = [UIFont systemFontOfSize:20]; // slogan字体
...
//                                                                  block:nil];
    ...
    viewModel.auxiliaryPrivacyWords = @[@"同意小世界", @"&", @"&", @" "];   // 条款之外的文案，默认可不设置
...
- (void)floatWindowLogin:(UIViewController *)controller {
    ...
    NSBundle *oneLogineBundle = [NSBundle bundleWithPath:[[NSBundle bundleForClass:[self class]] pathForResource:@"Onelogin" ofType:@"bundle"]];
    
    // 弹窗位置、大小设置，弹窗默认大小为300*340，居于屏幕中间，假如要弹窗居于底部，可做如下设置
    OLRect popupRect = {[self ol_screenHeight] - 370, 0, 0, 0, 0, 0, {[self ol_screenWidth], 370}};  // 弹窗偏移、大小设置
    viewModel.popupRect = popupRect;
    viewModel.popupCornerRadius = 0; // 弹窗圆角，默认为6
    UIImage *closeImage = [UIImage imageNamed:@"close" inBundle:oneLogineBundle compatibleWithTraitCollection:nil];
    viewModel.closePopupImage = closeImage; // 关闭按钮
    viewModel.closePopupTopOffset = @(20);  // 关闭按钮距弹窗顶部偏移
    viewModel.closePopupRightOffset = @(-16); // 关闭按钮距弹窗右边偏移
    
    viewModel.appLogo = [UIImage imageNamed:@"quick-login" inBundle:oneLogineBundle compatibleWithTraitCollection:nil];
    OLRect appLogoRect = {21, 0, 0, 0, 0, 0, {0, 20}};
    viewModel.logoRect = appLogoRect;
  
  // -------------- 自定义 title 下划线 -------------------
viewModel.customUIHandler = ^(UIView * _Nonnull customAreaView) {
  UIView *titleBottomLine = [[UIView alloc] initWithFrame:CGRectMake(0, 0, [self ol_screenWidth], 0.5)];
  titleBottomLine.backgroundColor = [UIColor colorWithRed:229 / 255.0 green:229 / 255.0 blue:229 / 255.0 alpha:1.0];
  [customAreaView addSubview:titleBottomLine];
  titleBottomLine.center = CGPointMake(customAreaView.bounds.size.width/2, 40);
  
};
  viewModel.phoneNumColor = [UIColor blackColor];
  viewModel.phoneNumFont = [UIFont fontWithName:@"PingFangSC-Medium" size:28.0];
  OLRect phoneNumReact = {100, 0, 0, 0, 0, 0, {0, 0}};
  viewModel.phoneNumRect = phoneNumReact;
  
  OLRect sloganRect = {147, 0, 0, 0, 0, 0, {0, 0}};  // slogan偏移、大小设置，偏移量和大小设置值需大于0，否则取默认值，默认可不设置
  viewModel.sloganRect = sloganRect;
  viewModel.sloganTextFont = [UIFont systemFontOfSize:12];
  viewModel.sloganTextColor = [UIColor colorWithRed:148/255.0 green:148/255.0 blue:148/255.0 alpha:1.0];
  
  OLRect authButtonRect = {194, 0, 0, 0, 0, 0, {315, 48}};  // 授权按钮偏移、大小设置，偏移量和大小设置值需大于0，否则取默认值，默认可不设置
  viewModel.authButtonRect = authButtonRect;
    UIImage *image = [UIImage imageNamed:@"one-login" inBundle:oneLogineBundle compatibleWithTraitCollection:nil];
  viewModel.authButtonImages = @[image, image, image];
  viewModel.authButtonTitle = [[NSAttributedString alloc] initWithString:@"" attributes:nil];
  viewModel.tapAuthBackgroundBlock = ^{
      NSLog(@"tapAuthBackgroundBlock");
  };
//    viewModel.customAuthActionBlock = ^{
//        NSLog(@"%@", [OneLogin currentNetworkInfo]);
//        NSDictionary *networkInfoFromCM = nil;
//        #pragma clang diagnostic push
//        #pragma clang diagnostic ignored "-Wundeclared-selector"
//            Class UASDKLogin = NSClassFromString(@"UASDKLogin");
//            if (UASDKLogin && [UASDKLogin respondsToSelector:@selector(shareLogin)]) {
//                id uaSdk = [UASDKLogin performSelector:@selector(shareLogin)];
//                if (uaSdk && [uaSdk respondsToSelector:@selector(networkInfo)]) {
//                    networkInfoFromCM = [uaSdk performSelector:@selector(networkInfo)];
//                }
//            }
//        #pragma clang diagnostic pop
//
//        if (networkInfoFromCM == nil || [[networkInfoFromCM objectForKey:@"networkType"] intValue] == 0) {
//            self.reject(@"-99999", @"no network", nil);
//        } else {
//            [OneLogin requestTokenWithCompletion:^(NSDictionary * _Nullable result) {
//                [self finishRequestingToken:result];
//            }];
//        }
//        return YES;
//    };
    if (self.privaryItems) {
        NSMutableArray *items = [[NSMutableArray alloc] init];
        if ([self.privaryItems objectForKey:@"userText"]) {
            OLPrivacyTermItem *item1 = [[OLPrivacyTermItem alloc] initWithTitle:[self.privaryItems objectForKey:@"secretText"]
                                                                        linkURL:[NSURL URLWithString:[self.privaryItems objectForKey:@"userUrl"]]
                                                                          index:-1];
            [items addObject:item1];
        }
        if ([self.privaryItems objectForKey:@"secretText"]) {
            OLPrivacyTermItem *item2 = [[OLPrivacyTermItem alloc] initWithTitle:[self.privaryItems objectForKey:@"secretText"]
                                                                        linkURL:[NSURL URLWithString:[self.privaryItems objectForKey:@"secretUrl"]]
                                                                          index:-1];
            [items addObject:item2];
        }
        
        viewModel.additionalPrivacyTerms = [items copy];
        OLRect termsRect = {286, 0, 0, 0, 0, 0, {[self ol_screenWidth] - 110, 0}};  // 服务条款偏移、大小设置，偏移量和大小设置值需大于0，否则取默认值，默认可不设置
        viewModel.termsRect = termsRect;
        viewModel.termTextColor = [UIColor colorWithRed:148/255.0 green:148/255.0 blue:148/255.0 alpha:1.0];
        viewModel.privacyTermsAttributes = @{
          NSForegroundColorAttributeName: [UIColor blackColor]
        };
        viewModel.termsAlignment = NSTextAlignmentCenter;
        if ([items count] == 0) {
            viewModel.auxiliaryPrivacyWords = @[@"登录代表您已阅读并同意 "];
        } else if ([items count] == 1) {
            viewModel.auxiliaryPrivacyWords =  @[@"登录代表您已阅读并同意 ", @" 和 "];
        } else if ([items count] == 2) {
            viewModel.auxiliaryPrivacyWords = @[@"登录代表您已阅读并同意 ", @" 和 ", @" 以及 ", @" "];
        }
    }
    OLRect checkboxRect = {0, 0, 0, 0, 0, 0, {0.01, 0.01}};
    viewModel.checkBoxRect = checkboxRect;
    viewModel.spaceBetweenCheckboxAndTermsText = 0.01;
    __weak typeof(self) wself = self;
    if ([self needPreGetToken]) {
//        [OneLoginProgressHUD showLoadingHUDWithMessage:nil];
        [OneLogin preGetTokenWithCompletion:^(NSDictionary * _Nonnull preResult) {
...
//                    flutterResult(@"0");
//                }
                [wself finishRequestingToken:preResult];
            }
        }];
    } else {
        [OneLogin requestTokenWithViewController:controller viewModel:viewModel completion:^(NSDictionary * _Nullable result) {
